...
// --- RENDER ENGINE ---
async function startQuartoRender(doc, jumpAfter) {
    ...
    const quartoProcess = (0, child_process_1.spawn)('quarto', args, { cwd: workspaceFolder });
    let hasError = false;
    let isCapturingErrorBlock = false;
    // Enhanced Multiline Error Capture
    const processOutput = (data, isStderr) => {
...
function getAllRelatedQmdFiles(mainQmdPath) {
    const files = new Set();
...
async function jumpToQmd(typEditor) {
    ...
    const textAround = typDoc.getText(new vscode.Range(typDoc.positionAt(Math.max(0, cursorOffset - 300)), typDoc.positionAt(cursorOffset + 300)));
    ...
    if (globalBestFile !== '' && globalHighScore > 1) {
        ...
        let targetEditor = vscode.window.visibleTextEditors.find(e => e.document.uri.fsPath === targetUri.fsPath);
        ...
        targetEditor.revealRange(new vscode.Range(cursorPos, cursorPos), vscode.TextEditorRevealType.InCenter);
